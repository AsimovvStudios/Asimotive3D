...
bool a3d_vk_debug_init(a3d *engine)
{
	PFN_vkCreateDebugUtilsMessengerEXT pfn_create =
		(PFN_vkCreateDebugUtilsMessengerEXT) vkGetInstanceProcAddr(
			engine->vk.instance, "vkCreateDebugUtilsMessengerEXT"
	);
	if (!pfn_create) {
	...
	VkResult result = pfn_create(engine->vk.instance, &info, NULL, &engine->vk.debug_messenger);
...
bool a3d_vk_debug_shutdown(a3d *engine)
{
	PFN_vkDestroyDebugUtilsMessengerEXT pfn_destroy =
		(PFN_vkDestroyDebugUtilsMessengerEXT)vkGetInstanceProcAddr(
			engine->vk.instance, "vkDestroyDebugUtilsMessengerEXT"
	);
	if (pfn_destroy && engine->vk.debug_messenger) {
		pfn_destroy(engine->vk.instance, engine->vk.debug_messenger, NULL);
		engine->vk.debug_messenger = VK_NULL_HANDLE;
